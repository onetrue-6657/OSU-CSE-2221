...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber[] partialProducts(NaturalNumber[] nnArray) {
        ...
        int len = nnArray.length;
        NaturalNumber[] partialProducts = new NaturalNumber[len];
        for (int i = 0; i < nnArray.length; i++) {
            partialProducts[i] = new NaturalNumber1L(1);
        }
        for (int i = 0; i < nnArray.length; i++) {
            for (int j = 0; j <= i; j++) {
                partialProducts[i].multiply(nnArray[j]);
            }
        }
