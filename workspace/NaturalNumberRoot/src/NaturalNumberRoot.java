...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        NaturalNumber lowEnough = new NaturalNumber1L(0); // lowEnough = 0
        NaturalNumber tooHigh = new NaturalNumber1L(n);
        tooHigh.increment(); // tooHigh = n + 1

        NaturalNumber diff = new NaturalNumber1L(tooHigh);
        diff.subtract(lowEnough); // diff = tooHigh - lowEnough

        while (diff.compareTo(one) > 0) { // while (diff > 1)
            NaturalNumber g = new NaturalNumber1L(lowEnough);
            g.add(tooHigh);
            g.divide(two); // g = (lowEnough + tooHigh) / 2
            NaturalNumber gCopy = new NaturalNumber1L();
            gCopy.copyFrom(g);
            gCopy.power(r);
            if (n.compareTo(gCopy) < 0) { // if (n < power(g, r))
                tooHigh.copyFrom(g); // tooHigh = g
                diff = new NaturalNumber1L(tooHigh);
                diff.subtract(lowEnough); // prepares for next loop condition check
            } else {
                lowEnough.copyFrom(g); // lowEnough = g
                diff = new NaturalNumber1L(tooHigh);
                diff.subtract(lowEnough); // same as above
            }
        }
