...
public final class NaturalNumberInstanceOps extends NaturalNumber2 {
    ...
    @Override
    public void power(int p) { // n^5 = (n^2.5)^2
        assert p >= 0 : "Violation of: p >= 0";

        if (p > 1) {
            NaturalNumber copy = new NaturalNumber1L(this);
            NaturalNumber copyOne = new NaturalNumber1L(this);
            NaturalNumber copyTwo = new NaturalNumber1L(this);
            copyOne.power(p / 2);
            copyTwo.power(p / 2);
            copyOne.multiply(copyTwo);
            this.copyFrom(copyOne);
            if (p % 2 != 0) {
                this.multiply(copy);
            }
        }
